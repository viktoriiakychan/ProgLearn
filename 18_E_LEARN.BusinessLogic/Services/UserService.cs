...
namespace _18_E_LEARN.BusinessLogic.Services
{
    public class UserService
    {
        ...
        public async Task<ServiceResponse> GetUserIdForEditingAsync(string userId)
        {
            var user = await _userManager.FindByIdAsync(userId);
            if (user == null)
            {
        ...
        {
            List<AppUser> users = await _userManager.Users.ToListAsync(); // get all users
            List<AllUsersVM> mappedUsers = users.Select(u => _mapper.Map<AppUser, AllUsersVM>(u)).ToList();
            //mappedUsers.Remove(mappedUsers.Where(x => x.Role == "Administrators"));
        ...
        public async Task<ServiceResponse> EditUserAsync(EditUserVM model)
        {
            ...
            {
                //user.LockoutEnd
                _userManager.SetLockoutEndDateAsync(user, DateTime.UtcNow.AddMinutes(5.0));
            }
            if (user.Email != model.Email)
            {
                user.Email = model.Email;
        ...
        public async Task<ServiceResponse> BlockAsync(string id)
        {
            ...
            user.LockoutEnd = DateTimeOffset.Now.AddMinutes(10.0);
        ...
        public async Task<ServiceResponse> UnblockAsync(string id)
        {
            var user = await _userManager.FindByIdAsync(id);
            if (user == null)
            {
            ...
            {
                Success = true,
                Message = "The user was unblocked",
                Payload = user
            };
        }
    }
...
// service --> infrastruture register
// show can be done in admin

// button => viewmodel
//edit add delete(what if there will be something !!!)
